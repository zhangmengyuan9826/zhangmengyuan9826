...
export default {
  ...
  methods: {
    ...
    reset_that(){
      that.imageSrc= null,
      this.checkQualifiedQuantity0= false
      that.form = {
        orderId: null,
        orderNo: null,
        orderType: null,
  ...
  beforeDestroy() {
    ...
    checkQualifiedQuantity(){
        for(let i = 0; i < this.inOrderDetailList.length; i++){
          console.log(this.inOrderDetailList[i].qualifiedQuantity)
    ...
    viewPdf(){
     ...
     /** 导入按钮操作 */
        ...
        handelUploadPdfSuccess(response){          
    ...
    confirmCheck() {
      if(! this.checkCoa()){
        Message.warning("入库单包含非预研物料，需上传COA图片或文件！")
        return 
      }
      let that = this;
      ...
      that.$modal.confirm("是否确认质检结果？").then(function () {
        if(!that.checkQualifiedQuantity() && !that.checkQualifiedQuantity0){
          that.$modal.confirm("物料合格数量为0，请确认！")
            .then(function () {
        ...
        console.log(that.checkQualifiedQuantity0)
        if(!that.checkQualifiedQuantity0 && !that.checkQualifiedQuantity){
          return
        }
