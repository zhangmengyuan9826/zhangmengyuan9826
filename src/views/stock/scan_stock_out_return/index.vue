<template>
 <div>
  <p>出库退货流程：出库退货单【ORDER】——选择项目——货位单【LOCATION】</p>
 ...
 <div v-if="ifscan"> 
    ...
    <div class="order">
			<div>单据类型：</div>
			<div class="label">{{order.returnTypeLabel}}</div>
			...
			<div v-for="(item, index) in order.detailList" :key="index">
				<div :class="[item.lineNo === selectItem.lineNo ? 'detail-item active' : 'detail-item']" @click="chooseItem(item)">
					...
					<div class="right">
						...
						<div>所在货位{{item.locationCode ? item.locationCode : ''}}</div>
					</div>
					<div class="tag">
						<el-tag text="" type="warning" v-if="item.quantity > item.returnQuantity">待完成</el-tag>
...
import { getInfoM,submitOutReturn  } from "@/api/stock/outReturn";
...
export default {
  ...
  methods: {
    ...
    loadStockOutReturnOrder(){
      getInfoM((this.order.returnNo)).then((res) => {
				if(res.code === 200){
					this.order = res.data;
				}else{
    ...
    confirm(){
			let that = this;
      console.log(that.locationCheck);
      if(! that.locationCheck){
        Message.warning("未扫码货位码！")
        return
      }
      this.$confirm('确认提交退货信息？', '提示', { type: 'warning' }).then(
        () => {
          // that.$refs.loadinToast.show({ ...that.toastParams });
            submitOutReturn(that.order).then((stockOutReturnRes) => {
							if(stockOutReturnRes.code === 200){
								that.order = [];
								// that.selectItem = {};
							}else {
                Message.error(stockOutReturnRes.msg)
              }
