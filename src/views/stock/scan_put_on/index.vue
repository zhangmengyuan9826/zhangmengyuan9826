<template>
 <div class="app-container">
  <p class="error">{{ error }}</p> 
      <!--错误信息-->
  <el-button type="primary" @click="reScan" :customStyle="'width:120px;height:64px;margin-left:10px;'">扫码上架</el-button>
...
import { submitPutOn } from "@/api/stock/info";
...
export default {
  ...
  methods: {
      ...
      getMatLabel(labelId).then((res)=>{
        if(res.code === 200){
					if(!res.data.locationCode){
						Message.warning('该物料未入库！')
						// this.$refs.loadinToast.hide();
						return;
					}
					this.matLabelList.unshift(res.data);
          that.selectLocation = this.selectLocation;
				}else{
    ...
    confirm(){
			let that = this;
      this.$confirm('确认提交上架信息?', '提示', { type: 'warning' }).then(
        () => {
						let putOnRequestBody = {
							warehouseCode: that.selectWarehouseCode,
							locationCode: that.selectLocation,
							matLabelList: that.matLabelList,
						};
            submitPutOn(putOnRequestBody).then((putOnRes) => {
