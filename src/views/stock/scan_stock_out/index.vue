<template>
 ...
 <div class="app-container">
  ...
  <p>出库流程：出库单【ORDER】——货位单【LOCATION】——选择物料——物料标签【LABEL】</p>
  <p class="decode-result">
          扫描结果:
          <b>{{ result }}</b>
      </p>
 ...
 <div v-if="ifscan"> 
			...
			<div v-for="(item, index) in order.detailList" :key="index">
				<div :class="[item.lineNo === selectItem.lineNo ? 'detail-item active' : 'detail-item']" @click="chooseItem(item)">
					...
					<div class="right">
						...
						<div>推荐货位：{{item.locationCode ? item.locationCode : ''}}</div>
					</div>
					<div class="tag">
						<el-tag text="" type="error" v-if="item.receivedQuantity === 0">待完成</el-tag>
						<el-tag text="" type="warning" v-if="item.quantity > item.receivedQuantity && item.receivedQuantity > 0">领料中</el-tag>
            <el-tag text="" type="success" v-if="item.quantity === item.receivedQuantity">已完成</el-tag>
					</div>
				</div>
    ...
    <!-- <el-button type="primary" @click="confirm" :customStyle="'width:120px;height:64px;margin-left:10px;'">提交</el-button> -->
		<el-dialog title="选择物料" :visible.sync="showModal" center showCancelButton>
            ...
            <el-input-number v-model="scanMatLabelItem.inputQuantity" :min="1" :max="scanMatLabelItem.remainingQuantity" :step="1" integer></el-input-number>
					</div>
				</div>
			</div>
       <span slot="footer" class="dialog-footer">
        <el-button @click="cancelSelectMatLabel">取 消</el-button>
        <el-button type="primary" @click="confirmSelectMatLabel">确定</el-button>
      </span>
		</el-dialog>
    </div>
...
export default {
 ...
 created(){
// this.onDecode("LOCATION:OP20240110154029")
      },
  methods: {
      onDecode(result) {
          this.result = result;
