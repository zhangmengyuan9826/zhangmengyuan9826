<template>
 ...
 <div v-if="ifscan"> 
    ...
    <el-button type="primary" @click="confirm" :customStyle="'width:120rpx;height:64rpx;margin-left:10rpx;'">提交</el-button>
		</div>
  ...
  methods: {
      onDecode(result) {
          ...
          if(result.indexOf("ORDER:") === 0){
            ...
            this.order.orderNo = 'IP20220809115240';
            this.ifscan=false;
        ...
        () => {
          // that.$refs.loadinToast.show({ ...that.toastParams });
            submitStockIn(that.order).then((stockInRes) => {
							...
							alert({ title: stockInRes.msg, icon: 'none' })
